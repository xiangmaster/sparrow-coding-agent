...
已经完成内部数据模型、工作区安全边界、十个本地工具、DeepSeek Provider、
...
轻量工具动作。Agent 的可见说明通过 DeepSeek SSE 增量写入同一个回复卡片，完整响应
仍会进入上下文和审计轨迹；同一任务可以连续追问，重启应用后也可从左侧“任务会话”恢复上下文。
...
预算终止回复会同时列出已确认的文件变化、最近验证结果、停止原因和继续方式。
