...
Provider 配置与代码工作区具有不同的信任边界。API Key 只从启动 Sparrow 时所在目录的
`.env` 加载；用户随后选择的目标仓库只提供文件、命令和轨迹存储空间。切换工作区不会
改变模型配置，也不会因为目标仓库自带 `.env` 而覆盖 Sparrow 的凭据或 API 地址。
...
7. 达到最大迭代数、反复执行相同调用、用户中断、Provider 无法恢复或预算耗尽时强制终止。
   累计 Token 预算默认 `400000`，GUI 和 CLI 均可显式调整；它限制整次运行成本，
   不等同于单次请求的上下文窗口。
